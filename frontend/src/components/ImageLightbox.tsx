 ...
 * needs a "click to see it full-size" affordance without leaving the current page. */
export function ImageLightbox({ open, onClose, title, imageUrl, subtitle }: Props) {
  return (
    <Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth PaperComponent={ResizableDialogPaper}>
      ...
      >
        <Box
          ...
          src={imageUrl}
          alt={title}
          sx={{ maxWidth: '100%', maxHeight: '75vh', display: 'block', objectFit: 'contain' }}
        />
      </DialogContent>
    </Dialog>
  );
